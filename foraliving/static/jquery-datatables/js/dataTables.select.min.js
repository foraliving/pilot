...
(function (e) {
    ...
    function v(b, c, a) {
        ...
        var f =
            function (a, c) {
                ...
                if (d.indexOf(a) > d.indexOf(c))var f = c, c = a, a = f;
                var e = !1;
        ...
        !b.cells({selected: !0}).any() && !a ? (d = d(0, c.column), a = f(0, c.row)) : (d = d(a.column, c.column), a = f(a.row, c.row));
        a = b.cells(a, d).flatten();
        b.cells(c, {selected: !0}).any() ? b.cells(a).deselect() : b.cells(a).select()
    }
    ...
    function x(b) {
        var c = e(b.table().container()), a = b.settings()[0], d = a._select.selector;
        c.on("mousedown.dtSelect", d, function (a) {
            ...
            if (!j.getSelection || !j.getSelection().toString()) {
                var d = b.settings()[0];
                if (e(a.target).closest("div.dataTables_wrapper")[0] ==
                    b.table().container()) {
                    var g = b.cell(e(a.target).closest("td, th"));
                    if (g.any()) {
                        var h = e.Event("user-select.dt");
                        i(b, h, [c, g, a]);
                        h.isDefaultPrevented() || (h = g.index(), "row" === c ? (c = h.row, s(a, b, d, "row", c)) : "column" === c ? (c = g.index().column, s(a, b, d, "column", c)) : "cell" === c && (c = g.index(), s(a, b, d, "cell", c)), d._select_lastCell = h)
                    }
                }
            }
        });
        e("body").on("click.dtSelect", function (c) {
            a._select.blurable && !e(c.target).parents().filter(b.table().container()).length && (e(c.target).parents("div.DTE").length || p(a, !0))
        })
    }

    function i(b,
               c, a, d) {
        if (!d || b.flatten().length)"string" === typeof c && (c += ".dt"), a.unshift(b), e(b.table().node()).triggerHandler(c, a)
    }

    function y(b) {
        var c = b.settings()[0];
        if (c._select.info && c.aanFeatures.i) {
            var a = e('<span class="select-info"/>'), d = function (c, d) {
    ...
    function z(b, c, a, d) {
        var f = b[c + "s"]({search: "applied"}).indexes(), d = e.inArray(d, f), m = e.inArray(a, f);
        if (!b[c + "s"]({selected: !0}).any() && -1 === d)f.splice(e.inArray(a, f) + 1, f.length); else {
            if (d > m)var k = m, m = d, d = k;
    ...
    function s(b, c, a, d, f) {
        var e = c.select.style(), k = c[d](f, {selected: !0}).any();
        "os" === e ? b.ctrlKey || b.metaKey ? c[d](f).select(!k) : b.shiftKey ? "cell" === d ? v(c, f, a._select_lastCell || null) : z(c, d, f, a._select_lastCell ? a._select_lastCell[d] : null) : (b = c[d + "s"]({selected: !0}), k && 1 === b.flatten().length ? c[d](f).deselect() : (b.deselect(), c[d](f).select())) : "multi+shift" == e ? b.shiftKey ?
            "cell" === d ? v(c, f, a._select_lastCell || null) : z(c, d, f, a._select_lastCell ? a._select_lastCell[d] : null) : c[d](f).select(!k) : c[d](f).select(!k)
    }
    ...
    g.select = {};
    g.select.version = "1.2.1";
    g.select.init = function (b) {
        var c = b.settings()[0], a = c.oInit.select, d = g.defaults.select, a = a === h ? d : a, d = "row", f = "api", m = !1, k = !0, w = "td, th", j = "selected", i =
            !1;
        c._select = {};
        if (!0 === a)f = "os", i = !0; else if ("string" === typeof a)f = a, i = !0; else if (e.isPlainObject(a) && (a.blurable !== h && (m = a.blurable), a.info !== h && (k = a.info), a.items !== h && (d = a.items), a.style !== h && (f = a.style, i = !0), a.selector !== h && (w = a.selector), a.className !== h))j = a.className;
        b.select.selector(w);
        b.select.items(d);
        b.select.style(f);
        b.select.blurable(m);
        ...
        e.fn.dataTable.ext.order["select-checkbox"] = function (a, c) {
            return this.api().column(c, {order: "index"}).nodes().map(function (c) {
                return "row" ===
                a._select.items ? e(c).parent().hasClass(a._select.className) : "cell" === a._select.items ? e(c).hasClass(a._select.className) : !1
            })
        };
    ...
    e.each([{type: "row", prop: "aoData"}, {type: "column", prop: "aoColumns"}], function (b, c) {
        g.ext.selector[c.type].push(function (a, b, f) {
            var b = b.selected, e, k = [];
            if (b === h)return f;
            for (var g = 0, i = f.length; g < i; g++)e = a[c.prop][f[g]], (!0 === b && !0 === e._select_selected || !1 === b && !e._select_selected) && k.push(f[g]);
            return k
        })
    });
    ...
    n("select.blurable()", function (b) {
        return b === h ? this.context[0]._select.blurable :
    ...
    n("select.style()", function (b) {
        return b === h ? this.context[0]._select.style : this.iterator("table", function (c) {
            ...
            if (!c._select_init) {
                ...
                a.on("preXhr.dt.dtSelect", function () {
                    var c = a.rows({selected: !0}).ids(!0).filter(function (a) {
                        return a !== h
                    }), b = a.cells({selected: !0}).eq(0).map(function (c) {
                        var b = a.row(c.row).id(!0);
                        return b ?
                        {row: b, column: c.column} : h
                    }).filter(function (a) {
                        return a !== h
                    });
    ...
    n("select.selector()", function (b) {
        return b === h ? this.context[0]._select.selector : this.iterator("table",
            function (c) {
                r(new g.Api(c));
    ...
    o("columns().deselect()", "column().deselect()", function () {
        ...
        this.iterator("column", function (c, a) {
            ...
            var b = new g.Api(c), f = b.column(a);
            ...
            b.cells(null, a).indexes().each(function (a) {
                var b = c.aoData[a.row], d = b._selected_cells;
    ...
    o("cells().deselect()", "cell().deselect()",
        function () {
            var b = this;
    ...
    e.extend(g.ext.buttons, {
        selected: {
            text: q("selected", "Selected"), className: "buttons-selected", init: function (b, c, a) {
                var d = this;
                a._eventNamespace = ".select" + u++;
                b.on(t(a), function () {
                    var a = d.rows({selected: !0}).any() || d.columns({selected: !0}).any() ||
        ...
        selectedSingle: {
            ...
            init: function (b, c, a) {
                ...
                a._eventNamespace = ".select" + u++;
                b.on(t(a), function () {
                    var a = b.rows({selected: !0}).flatten().length + b.columns({selected: !0}).flatten().length + b.cells({selected: !0}).flatten().length;
                    d.enable(1 === a)
                });
                this.disable()
            },
    ...
    e(l).on("preInit.dt.dtSelect", function (b, c) {
